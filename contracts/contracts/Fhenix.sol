...
contract GnomesAndKnights {
    ...
    function createProfile(address _player, string calldata _nfc) public {
        ...
        for (uint8 i = 1; i < BOARD_SIZE; i++) {
            for (uint8 j = 0; j < BOARD_SIZE; j++) {
                newPlayer.grid[i][j] = FHE.asEuint8(uint8(_random[i + j]) % 17);
            }
        }
        newPlayer.grid[0][0] = FHE.asEuint8(0);
        newPlayer.grid[0][1] = FHE.asEuint8(0);
        newPlayer.grid[0][2] = FHE.asEuint8(0);
        newPlayer.grid[0][3] = FHE.asEuint8(0);
    ...
    function addTwoMatrices(
        ...
        for (uint8 i = 0; i < BOARD_SIZE; i++) {
            for (uint8 j = 0; j < BOARD_SIZE; j++) {
                result[i][j] = FHE.add(a[i][j], b[i][j]);
            }
        }
        return result;
    }
